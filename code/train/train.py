...
def main(args):
    ...
    data = pd.read_csv('https://raw.githubusercontent.com/kiseli98/ai_lab3/main/apartmentComplexData%20-%20Copy.csv',
                       names=column_names)
    ...
    run.log('Coefficients', lr_model.coef_)
    run.log('Accuracy', mean_squared_error(test_labels, prediction))
    run.log('Accuracy', r2_score(test_labels, prediction))
    ...
    run.log('Predicted value', prediction)



    # files saved in the "outputs" folder are automatically uploaded into run history
    model_file_name = "model.pkl"
    joblib.dump(lr_model, os.path.join('outputs', model_file_name))
